...
class Graph:
    ...
    def max_degree_nodes(self) -> dict:
        ...
        for n in self.nodes:
            for i in range(len(self.edges)):
             if self.edges[i][0]==n[0] or self.edges[i][1]==n[0]:
    ...
    def num_leaf_nodes(self):
        counts={}
        ids_list1 = [e[0] for e in self.edges]
        ids_list2 = [e[1] for e in self.edges]
        for x in self.nodes:
            count=ids_list1.count(str(x[0]))+ids_list2.count(str(x[0])) 
            counts[x[0]]=count
...
class  TMDBAPIUtils:
    ...
    def get_movie_cast(self, movie_id:str, limit:int=None, exclude_ids:list=None) -> list:
        ...
        conn.request("GET", "/3/movie/{movie_id}/credits?api_key={api_key}&language=en-US".format(movie_id=movie_id,api_key=self.api_key), payload, headers)
        res = conn.getresponse()
        #print("cast")
        actual_response = res.read()
        ac=actual_response.decode()
        if "cast" in json.loads(ac):
            
            movie_list1 =json.loads(ac)["cast"]
            #print(movie_list1)
            cast_members=[]
            for m in movie_list1:
                if limit!=None:
                    if limit> m["order"]:

                      cast_members.append(m)
                else:
                      cast_members.append(m)

            es= [c["id"] for c in cast_members]
            if exclude_ids:
                    for e in range(len(exclude_ids)):
                            ind= es.index(exclude_ids[e])
                            cast_members.remove(cast_members[ind])
    ...
    def get_movie_credits_for_person(self, person_id:str, vote_avg_threshold:float=None)->list:
        ...
        #headers = {}
        headers = {'Content-type': 'application/json'}
        conn.request("GET", "/3/person/{person_id}/movie_credits?api_key={api_key}&language=en-US".format(person_id=person_id,api_key=self.api_key), payload, headers)
        ...
        for m in movie_list:
                    ...
                    if vote_avg_threshold != None : 
                        if "vote_average" in m:
                            #print("present in" ,m)
                            if m["vote_average"]>=vote_avg_threshold:
                            # print("found")
                                movie_credits.append(m)
...
def return_argo_lite_snapshot()->str:
    ...
    """
    return "https://poloclub.github.io/argo-graph-lite/#8ae7d4c8-bc77-4cc9-85c7-e9d84d6ea9cf"
...
if __name__ == "__main__":
    ...
    tmdb_api_utils = TMDBAPIUtils(api_key='9956b3952d92aad6591fad935ec6cce0')
    ...
    while i<3:
        for n in nodes:
            credits1 = tmdb_api_utils.get_movie_credits_for_person(n[0], 8.0)
            for c1 in credits1:
                actors = tmdb_api_utils.get_movie_cast(c1['id'], 3)
                for a in actors:
                    n1 = (str(a["id"]),a["name"])
                    new_nodes.append(n1)
                    graph.add_node(id=str(a["id"]), name=a["name"].replace(",",""))
                    graph.add_edge(n[0], n1[0])
        i +=1
        nodes = new_nodes
        new_nodes = []
    ...
    # graph = Graph(with_edge_file="edges.csv", with_nodes_file="nodes.csv")
